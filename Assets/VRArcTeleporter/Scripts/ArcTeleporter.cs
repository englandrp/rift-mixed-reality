...
namespace VRInteraction
{
	public class ArcTeleporter : MonoBehaviour 
	{
		...
		protected virtual void Start ()
		{
			...
			if (input == null) input = GetComponent<VRInputOculus>();
			if (input == null) Debug.LogWarning("Arc Teleporter requires an Input script", gameObject);
			...
			if (_teleportHighlightInstance != null && _teleportHighlightInstance.activeSelf)
			{
				...
				else
				{
					if (_roomShapeInstance == null)
						...
						_teleportHighlightInstance.transform.rotation = Quaternion.LookRotation(_destinationNormal, Quaternion.FromToRotation(_vrPlayArea.transform.forward, _roomShapeInstance.transform.forward) * Camera.main.transform.forward);
				}
			}
		...
		private void CalculateArc()
		{
			...
			case ArcImplementation.PHYSICS_ARC:
				...
				if (offsetTrans != null)
				{
					initVel = offsetTrans.forward*(initialVelMagnitude*_vrPlayArea.localScale.x);
				}
