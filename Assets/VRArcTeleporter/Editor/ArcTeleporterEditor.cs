...
namespace VRInteraction
{
	...
	public class ArcTeleporterEditor : Editor
	{
		...
		public virtual void OnEnable()
		{
			...
			IVRInput input = teleporter.GetComponent<VRInput>();
			if (input == null) input = teleporter.GetComponent<VRInputOculus>();
			if (input != null && input.getVRActions == null) SetDefaultActions(input);
		}

		private void SetDefaultActions(IVRInput input)
		{
			string[] premadeActions = {"NONE", "SHOW", "TELEPORT"};
			input.g_triggerKey = 2;
			input.g_triggerKeyOculus = 2;
			input.g_padTop = 1;
			input.g_padTopOculus = 0;
			input.g_padLeft = 1;
			input.g_padLeftOculus = 0;
			input.g_padRight = 1;
			input.g_padRightOculus = 0;
			input.g_padBottom = 1;
			input.g_padBottomOculus = 0;
			input.g_padCentre = 1;
			input.g_padCentreOculus = 0;
			input.g_gripKey = 0;
			input.g_gripKeyOculus = 0;
			input.g_menuKey = 0;
			input.g_menuKeyOculus = 0;
			input.g_aButtonKey = 0;
			input.g_aButtonKeyOculus = 1;
			input.getVRActions = premadeActions;
			EditorUtility.SetDirty(input.getGameObject());
		}
		...
		public override void OnInspectorGUI()
		{
			...
			if (input == null)
			{
				EditorGUILayout.HelpBox("Arc Teleporter requires either VRInput or VRInputOculs component on this gameobject", MessageType.Warning);
				return;
			}
			...
			if (controlSchemeEnum == ArcTeleporter.ControlScheme.PRESS_AND_RELEASE)
			{
				...
				if (showIndex != -1)
				{
					if (input.g_triggerKey == showIndex || input.g_padCentre == showIndex || input.g_padTop ==showIndex ||
						input.g_padLeft == showIndex || input.g_padRight == showIndex || input.g_padBottom == showIndex ||
						input.g_gripKey == showIndex || input.g_menuKey == showIndex || input.g_aButtonKey == showIndex)
					{
